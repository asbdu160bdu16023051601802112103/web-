...
<div class="container">
    ...
    <div class="double-tap-info">
        ...
        <p><strong>Double-tap anywhere</strong>: Find location + Check safety + Start recording + Alarm</p>
...
function initApp() {
    ...
    setTimeout(() => {
        ...
        speak("SafeGuard is ready. Double tap to find your location and check safety, single tap to stop and save, triple tap to check location safety and activate both cameras.");
    }, 1000);
}
    ...
    if (currentTime - lastTapTime < TAP_TIME_THRESHOLD &&
        ...
        if (tapCount === 2) {
            ...
            doubleTapTimeout = setTimeout(() => {
                ...
                // Activate double tap emergency (with location safety check)
...
// ====== DOUBLE TAP EMERGENCY (Modified with Location Safety Check) ======
function activateDoubleTapEmergency() {
    console.log("🚨 DOUBLE TAP EMERGENCY ACTIVATED - Finding location and checking safety");
    ...
    // 1. First find and check location safety
    findAndCheckLocationSafety(() => {
        // 2. Then start recording video
        startRecording();
        ...
        // Show notification
        showNotification("Double tap emergency activated! Location checked, recording started, alarm activated!", true);
        
        // Voice announcement
        speak("Emergency activated! Your location has been checked. Video recording started. Police alarm activated.");
    });
}

// ====== FIND AND CHECK LOCATION SAFETY ======
function findAndCheckLocationSafety(callback) {
    console.log("📍 Finding and checking location safety...");
    
    // Update GPS status
    elements.gpsStatus.innerHTML = '<i class="fas fa-sync-alt fa-spin"></i> Finding location for safety check...';
    
    if (!navigator.geolocation) {
        elements.gpsStatus.innerHTML = '<i class="fas fa-exclamation-triangle"></i> GPS Not Supported';
        elements.locationDisplay.textContent = "GPS not supported";
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Get address and check safety
            getAddressFromCoordinates(lat, lng, (address) => {
                // Check safety of this location
                checkSafetyOfCurrentLocation(lat, lng, address);
                
                // Show notification
                showNotification("Location found! Safety check completed.");
        ...
        (error) => {
            ...
            // Still execute callback even if location fails
            if (callback) callback();
        },
        {
...
function checkSafetyOfCurrentLocation(lat, lng, address) {
    console.log("🔍 Checking safety of current location...");
    ...
    setTimeout(() => {
        ...
        speak(`Safety check complete. Your current location is ${safetyLevel} with a rating of ${safetyScore} out of 10.`);
